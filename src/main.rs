...
fn main() -> Result<(), Error> {
    logger::init()?;
    let mut connection = database::init("squig.db")?;

    let transaction = connection.transaction()?;
    for site in requests::sites::call()? {
        database::sites::insert_or_ignore_into(&transaction, &site.name, &site.username)?;
        let site_id = database::sites::select_id_from(&transaction, &site.name, &site.username)?;
        for db in site.dbs {
        ...
        for brand in requests::brands::call(&squig.username, &squig.folder)? {
            ...
            for phone in brand.phones {
                ...
                for text in texts {
                    ...
                    let file_id = database::files::select_id_from(&transaction, phone_id, &text)?;
